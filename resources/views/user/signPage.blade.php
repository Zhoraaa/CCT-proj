...
@section('body')
    <form action="{{ @route('signUp') }}" method="POST" class="border border-secondary rounded m-2 p-3">
        @csrf
        <h1>Регистрация</h1>
        ...
        <div class="form-block-wrapper"><input type="password" name="password_confirmation" placeholder="Повтор пароля..."
                pattern="^(?=.*[A-Z])(?=.*[0-9])(?=.*[\W_]).{6,32}$"required>
        </div>
